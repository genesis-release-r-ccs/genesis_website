...
# 11.1 Coarse-grained MD simulation of protein with the AICG2+ model
...
[github](https://github.com/noinil/genesis_cg_tool) and download it with `git`:
...
[Julia](https://julialang.org/). Therefore, you have to first
[install](https://julialang.org/downloads/) and learn some [basic operations of Julia](https://docs.julialang.org/en/v1/manual/getting-started/). Please
...
You can also run the following command to install dependencies:
...
([tutorial22-11.1.tar.gz](/assets/tutorial_files/2022_03_tutorial22-11.1.tar.gz)). This tutorial has three steps: 1) system setup, 2) MD
...
the main topology file and integrates (see the "`#include`" lines in the file) parameters in the "`1PGB_cg.itp`" and other standard parameter
files. Please refer to [this webpage](https://github.com/noinil/genesis_cg_tool/wiki/File-formats)
...
In the "`param/`" directory there are standard parameter files (you can find a copy from the GENESIS-CG-tool).  The file "`pro.inp`" is the
...
pair-list distance for the non-native contacts (represented by the     excluded volume potential `cg_pairlistdist_exv`). You can consider
...
each interaction term, such as "`NATIVE_CONTACT`" (Go-like native contact interactions) and "`CG_EXV`" (excluded volume interactions).
...
```
Clementi, C., Nymeyer, H. & Onuchic, J. N., **2000**, *Journal of     molecular biology* 298, 937--953.
:::
