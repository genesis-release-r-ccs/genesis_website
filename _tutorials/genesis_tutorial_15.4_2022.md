...
\\( \\displaystyle \\left\[ -\\frac{1}{2} \\sum\_{i=1}\^f \\frac{\\partial\^2}{\\partial Q_i\^2} + V(\\mathbf{Q}) \\right\]\\Psi_n(\\mathbf{Q}) = E_n \\Psi_n(\\mathbf{Q}), \\)
...
\\( \\displaystyle V(\\mathbf{Q}) \\simeq V_0 + \\sum\_{i=1}\^f c\_{ii} Q_i\^2, \\)
...
[SINDO](https://tms.riken.jp/en/research/software/sindo/ "SINDO") program. SINDO is used to generate the
...
([tutorial22-15.4.tar.gz](fundamental/2022_06_tutorial22-15.4.tar.gz "tutorial22-15.4.tar.gz") or [github](https://github.com/yagikiyoshi/QMMMtutorial "github")), unzip
...
atomic charges. Note that this option **[must]{.has-inline-color .has-vivid-red-color}** be present if `macro=YES` in `[MINIMIZE]` (see below).
...
See [Appendix A](/tutorials/genesis_tutorial_15.4_2022/#appendixA) for further details
...
[`Appendix C`](/tutorials/genesis_tutorial_15.4_2022/#Appendix_C_How_to_download_and_compile_SINDO "Appendix C")
...
requires SINDO program, first see [Appendix C](/tutorials/genesis_tutorial_15.4_2022/#Appendix_C_How_to_download_and_compile_SINDO "Appendix C")
...
\\( \\displaystyle V\^{\\mathrm{QFF}}(\\mathbf{Q}) = V_0 + \\sum\_{i=1}\^f c\_{ii} Q_i\^2 + \\sum\_{i, j, k}\^f c\_{ijk} Q_iQ_jQ_k + \\sum\_{i, j, k,l}\^f c\_{ijkl} Q_iQ_jQ_kQ_l, \\)

and the 1D-grid PES is,

\\( \\displaystyle V\^{\\mathrm{1D-grid}} (\\mathbf{Q}) = \\sum\_{i=1}\^f V_i(Q_i) \\)
...
\\( \\displaystyle V\^{\\mathrm{QFF'}}(\\mathbf{Q}) = V\^{\\mathrm{QFF}} -- \\sum\_{i=1}\^f \[ c\_{ii} Q_i\^2 + c\_{iii} Q_i\^3 + c\_{iiii} Q_i\^4\]. \\)
...
input file to carry out VQDPT2 calculations. See [SINDO website](https://tms.riken.jp/en/research/software/sindo "SINDO website")
...
[](https://pubs.acs.org/doi/10.1021/jp048617g)\
[^3] J. VandeVondele, P. Troster, P. Tavan, and G. Mathias, J. Phys.
Chem. A **116**, 2466−2474 (2012).
[](https://pubs.acs.org/doi/10.1021/jp211783z)\
[^4] K. Yagi, S. Hirata, and K. Hirao, Theor. Chem. Acc. **118**,
681-691 (2007).
[](https://link.springer.com/article/10.1007/s00214-007-0363-x)\
[^5] K. Yagi, S. Hirata, and K. Hirao, Phys. Chem. Chem. Phys. **10**,
1781-1788 (2008).
[](https://pubs.rsc.org/en/content/articlelanding/2008/CP/b719093j)\
[^6] K. Yagi and H. Otaki, J. Chem. Phys. **140**, 084113 (2014).
[](https://aip.scitation.org/doi/10.1063/1.4866365)
...
[SINDO](https://tms.riken.jp/en/research/software/sindo "SINDO"), where further information is available.
